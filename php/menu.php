...
if ($act == "add_menu") {
    ...
    $Price = $_POST["Price"];
    $target_dir = "assets/uploadfood/";
    $img = basename($_FILES["imgMenu"]["name"]); //เก็บชื่อไฟล์รูปที่อัพโหลด
    $location = $target_dir . $img;
    $fileImages = $_FILES["imgMenu"]["tmp_name"]; //เก็บตำแหน่งของรูปที่อัพโหลด

    $imageFileType = strtolower(pathinfo($img, PATHINFO_EXTENSION)); //เลือกประเภทไฟล์
    if ($img != null) {
        /* Check file extension */
        if (in_array($imageFileType, $valid_extensions)) {
            /* Upload file */
            if (move_uploaded_file($fileImages, $location)) {
                $sql = "INSERT INTO Menu (menuType, menuName, Price, imgMenu) VALUES ('$menuType','$menuName','$Price','$img')";
...
if ($act == "edit_menu") {
    ...
    if ($img_edit == null) {
        $sql_img = "UPDATE `Menu` SET menuType = '$menuType', menuName = '$menuName', Price = '$Price' WHERE id = '$id'";
        echo updateDB($sql_img, "edit", $conn);
        // die ($sql_img);
    } else {  //เมื่อแก้ไขรูปภาพ
        $path = "assets/uploadfood/";
        ...
        if (in_array($ext, $valid_extensions)) {
            //ดึงข้อมูลรูป
            $selectimg = "SELECT `imgMenu` FROM `Menu` WHERE id= $id";
            $result = json_decode(getDB($selectimg,$conn));

            if($result->status == 1){
                //ลบรูปภาพเก่าที่เคยลงไว้
                if(unlink($path.$result->data[0]->imgMenu)){
                    if(move_uploaded_file($tmp,$path.$img_edit)){
                        $sql = "UPDATE `Menu` SET menuType = '$menuType', menuName = '$menuName', Price = '$Price', imgMenu = '$img_edit' WHERE id = '$id'";
